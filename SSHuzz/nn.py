...
coverage_set = set()
correspond_dict = dict() # {edge_id, set(correspond_id)}
trans_dict = dict() # {edge_id, idx}
# get binary argv
argvv = sys.argv[1:]
...
def log(*msg):
    print("nn.py: ", time.asctime(time.localtime(time.time())), msg)
...
def process_data():
    ...
    log("afl showmap start")

    for f in seed_list:
        tmp_list = []
        try:
            # append "-o tmp_file" to strip's arguments to avoid tampering tested binary.
            if 'strip' in argvv[0] or 'mp3gain' in argvv[0]:
                out = call(['./afl-showmap', '-q', '-e', '-o', '/dev/stdout', '-m', '512', '-t', '500'] + argvv + [f] + ['-o', 'tmp_file'])
            elif 'tiff2bw' in argvv[0] or 'exiv2' in argvv[0]:
                out = call(['./afl-showmap', '-q', '-e', '-o', '/dev/stdout', '-m', '512', '-t', '500'] + argvv + [f] + ['/dev/null'])
            else:
        ...
        for line in out.splitlines():
            ...
            coverage_set.add(int(edge))
        raw_bitmap[f] = tmp_list
    counter = Counter(tmp_cnt).most_common()

    log("afl showmap finish")
    ...
    trans_dict = edge_number_index(cal_binary_val(bitmap, label))

    log("init bitmap finish")
...
def cal_binary_val(bitmap, label):
	r, c = len(bitmap), len(bitmap[0])
	val_list = ['' for _ in range(c)]
	for i in range(r):
		for j in range(c):
			val_list[j] = val_list[j] + str(int(bitmap[i][j]))
	res = []
	for i in range(c):
		res.append((val_list[i], label[i]))
	return res


def edge_number_index(count_list):
    tmp_list = sorted(count_list, key=lambda x: x[0])
    tmp_set = set()
    edge_list = list()
    for val, edge in tmp_list:
        if val not in tmp_set:
            edge_list.append(edge)
            tmp_set.add(val)
    trans_dict = dict()
    for idx, i in enumerate(edge_list):
        if i not in trans_dict:
            trans_dict[i] = idx
    return trans_dict
...
class LossHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, batch, logs={}):
        ...
        log(step_decay(len(self.losses)))
...
def splice_seed(fl1, fl2, idxx):
    ...
    while ret == 1:
        ...
        if lenn == 1:
            randf = random.choice(seed_list)
            tmp1 = open(randf, 'rb').read()
...
def select_edge_set(edge_num):
    if(np.random.random() < strategy_probability):
    # basic random choice
        return np.random.choice(MAX_BITMAP_SIZE, edge_num)
    else:
        edge_selection = list()
        for edge in coverage_set:
            if check_select_edge(edge) and edge in trans_dict:
                edge_selection.append(trans_dict[edge])
        select_list = list()
        if len(edge_selection) < edge_num:
            select_list = np.random.choice(edge_selection, edge_num, replace=True)
        else:
            select_list = np.random.choice(edge_selection, edge_num, replace=False)
        return select_list


def check_select_edge(edge_id):
    if edge_id not in correspond_dict:
        return True
    count = 0
    for cp in correspond_dict[edge_id]:
        if cp in coverage_set:
            count += 1
    if float(count)/len(correspond_dict[edge_id]) > strategy_ratio:
        return False
    return True
...
def gen_mutate2(model, edge_num, sign):
    ...
    if len(new_seed_list) < edge_num:
        rand_seed1 = [new_seed_list[i] for i in np.random.choice(len(new_seed_list), edge_num, replace=True)] # 500
    else:
    ...
    interested_indice = select_edge_set(edge_num)
    ...
    with open('gradient_info_p', 'w') as f:
        for idxx in range(len(interested_indice[:])):
            ...
            log("number of feature " + str(idxx))
...
def setup_server():
    ...
    log('connected by neuzz execution moduel ' + str(addr))
    gen_grad(b"train")
...
def init_corresponding_edges():
    correspond_dict = dict()
    with open('edge-{}'.format(argvv[0][argvv[0].find('/')+1:])) as f:
        for line in f.readlines():
            tmp = line.split(':')
            k = int(tmp[0])
            correspond_dict[k] = set()
            for e in tmp[1].split(','):
                correspond_dict[k].add(int(e))
    return correspond_dict

if __name__ == '__main__':
    correspond_dict = init_corresponding_edges()
    setup_server()
